...
for count_mice = 1:mice
        ...
        % choose low performance sessions between 60 and 80%
%         low_perf_sessions1= performance{count_mice} < network_performance_threshold;
%         low_perf_sessions2= performance{count_mice} < 0.6;
%         low_perf_sessions = low_perf_sessions2 - low_perf_sessions1;
%         
%         low_perf_sessions_indices= find(low_perf_sessions);
...
p = signrank(total_links_low.NOGO.TEX, total_links_high.NOGO.TEX)

%%
p = signrank(shared_links_low.NOGO.TEX, shared_links_high.NOGO.TEX)

%% changes shared and unique links are significant to predict performance levels

p = signrank(unique_links_low.NOGO.TEX, unique_links_high.NOGO.TEX)
