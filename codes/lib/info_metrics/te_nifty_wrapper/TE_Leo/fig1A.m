% Fig.1A
cd 'E:\data\texture_discrimination\mtp_12\mtp_12_TE\mtp_13'
...
for network_time_interval = {[1,1.5],[2,3.5],[3.5,6]}
    ...
    network_plot = plot(digraph(shared_subnetwork_high_perf_GO,channel_labels));
    ...
    plot(digraph(mean(network_adj_matrices_GO(:,:,high_perf_sessions),3)>= network_frequency_threshold,channel_labels),'EdgeColor','r','LineWidth',2,'XData', layout_spring_coordinates.X,'YData', layout_spring_coordinates.Y);
    plot(digraph(mean(network_adj_matrices_NOGO(:,:,high_perf_sessions),3)>= network_frequency_threshold,channel_labels),'EdgeColor','b','LineWidth',2,'XData', layout_spring_coordinates.X,'YData', layout_spring_coordinates.Y);
