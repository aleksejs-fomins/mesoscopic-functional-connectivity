...
% CUE=[1,1.5], TEX=[2,3.5], LICK=[3.5,6]
for network_time_interval = {[1,1.5],[2,3.5],[3.5,6]}
    
    shared_subnetwork_high_perf_GO = NaN([length(channel_labels_shared),length(channel_labels_shared),mice]);
    high_freq_GO = NaN([length(channel_labels_shared),length(channel_labels_shared),mice]);
    high_freq_NOGO = NaN([length(channel_labels_shared),length(channel_labels_shared),mice]);
    ...
    % create plot object for GO, selecting shared links among all mice
    network_plot = plot(digraph(all(shared_subnetwork_high_perf_GO,3),channel_labels_shared));
    % set node coordinates
    layout(network_plot,'circle')
    % set links properties
    network_plot.EdgeColor = 'black';
    
    % highlight links above frequency threshold (for all mice)
    hold on
    plot(digraph(all(high_freq_GO,3),channel_labels_shared),'Marker','none','EdgeColor','r','LineWidth',2,'layout', 'circle');
    plot(digraph(all(high_freq_NOGO,3),channel_labels_shared),'Marker','none','EdgeColor','b','LineWidth',2,'layout', 'circle');
